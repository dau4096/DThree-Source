...
from exct.artbook import searchArtbookIndexFile, getArtbookTags, searchArtbookIndexFileByList
...
async def checkReplies(messageData: str, message: discord.Message) -> None:
	...
	if messageData.startswith("/count "):
		...
		cleanedMessage:str = regex.sub(r'[^a-zA-Z0-9\s]', '', messageData);
		words:list[str] = cleanedMessage.split(" ");
		occurrencesUpdOccurrences(str(message.author), words);
	...
	if (messageData.startswith("/artbook ")):
		...
		if (len(tag) > 0):
			filePaths:list[str] = searchArtbookIndexFileByList([t.strip() for t in tag.split(",")]);
			if (len(filePaths) > 0):
				await message.reply(
	...
	if (messageData.startswith("/artbook-tags")):
		file:discord.File = getArtbookTags();
		await message.reply(content="All valid tags:", files=[file,], mention_author=True);
