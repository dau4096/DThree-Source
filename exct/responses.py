	...
	if messageData.startswith(f"/{fileName}+"):
		with open(f"/opt/render/project/src/textFiles/phrases/{fileName}.txt", "r", encoding="utf-8") as file:
			...
			if chosenLine == ":img:":
				...
				await replyMessage(message, chosenLine.replace("Â¬", "\n").replace("¬", "\n") + "\n Message may be negatively effected by current D3 testing performed by howitzer. /n For more information on how this can effect you and your discord experience, please contact your local D3 admin office.", ping=True)
				"""
